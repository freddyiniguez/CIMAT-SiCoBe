<?php 
    ...
    $var2 = $dependencia->selectMethod("selectbeneficiario");
...
<h2 class="title">Inscribir Persona a un Programa</h2>
<form class="form-horizontal" method="post" action="../controllers/controladorinscribirbeneficiario.php">
...
<!-- Select Basic -->
<div class="control-group">
  <label class="control-label" for="selBeneficiario">Beneficiario</label>
  <div class="controls">
    <select id="selBeneficiario" name="selBeneficiario" class="input-xlarge">
      <option>Selecciona el beneficiario</option>
      <?php
        foreach ($var2 as $key) {
          # code...
      ?>
      <option value="<?php echo $key['Curp'];?>"><?php echo utf8_encode($key['Nombre']);?></option>
      <?php
       }
      ?>
    </select>
...
<div class="control-group">
  ...
  <input type="hidden" name="bandera" id="bandera" value="inscribirbeneficiario">
